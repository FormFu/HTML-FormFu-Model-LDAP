# This Makefile is for the HTML::FormFu::Model::LDAP extension to perl.
#
...
#     ABSTRACT => q[Integrat FormFu and LDAP]
#     AUTHOR => q[Andreas Marienborg <andreas@startsiden.no>]
#     DIR => []
#     DISTNAME => q[HTML-FormFu-Model-LDAP]
#     NAME => q[HTML::FormFu::Model::LDAP]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], HTML::FormFu=>q[0.02000], Net::LDAP=>q[0] }
#     VERSION => q[0.0100]
...
NAME_SYM = HTML_FormFu_Model_LDAP
VERSION = 0.0100
...
FULLEXT = HTML/FormFu/Model/LDAP
BASEEXT = LDAP
PARENT_NAME = HTML::FormFu::Model
DLBASE = $(BASEEXT)
VERSION_FROM = 
OBJECT = 
LDFROM = $(OBJECT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/HTML/FormFu/Model/LDAP.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/HTML/FormFu/Model
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/HTML/FormFu/Model
...
TO_INST_PM = lib/HTML/FormFu/Model/LDAP.pm

PM_TO_BLIB = lib/HTML/FormFu/Model/LDAP.pm \
	blib/lib/HTML/FormFu/Model/LDAP.pm
...
DISTNAME = HTML-FormFu-Model-LDAP
DISTVNAME = HTML-FormFu-Model-LDAP-0.0100
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/HTML/FormFu/Model/LDAP.pm $(INST_MAN3DIR)/HTML::FormFu::Model::LDAP.$(MAN3EXT) 
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,0100,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Integrat FormFu and LDAP</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Andreas Marienborg &lt;andreas@startsiden.no&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="HTML-FormFu" VERSION="0,02000,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Net-LDAP" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/HTML/FormFu/Model/LDAP.pm blib/lib/HTML/FormFu/Model/LDAP.pm 
